...
if (navigator.userAgent.toLowerCase().indexOf('micromessenger') !== -1) {
    ...
    var divContent = [
        ...
        '    <span style="font-size: 14px; color: #999; line-height: 1.6;">你正在使用微信打开该网页。微信对部分网页恶意屏蔽，为维护自由上网环境，本页面禁止使用微信打开。使用浏览器打开本页面不会出现此提示。</span>',
        '</div>',
        '<div style="height: 25px;"></div>',
        '<div id="fakeWechatStyleButton" style="display: inline-block; border: none; border-radius: 5px; background-color: #1aad19; color: #ffffff; border: 1px solid rgba(0, 0, 0, 0.1); width: calc(100% - 30px); height: 46px; line-height: 46px; font-size: 18px; user-select: none;">',
        '    恢复是不可能恢复的',
        '</div>',
